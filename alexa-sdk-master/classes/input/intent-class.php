	...
	public function get_slots() {
		if( empty( $this->slots ) ) {
			...
			foreach( $object_vars AS $object_var ) {
				if (!empty($object_var->value)){
					$this->slots[$object_var->name]=$object_var->value;
				}
				if (isset($object_var->resolutions->resolutionsPerAuthority[0]->values[0]->value->name)){
					$this->slots[$object_var->name]=$object_var->resolutions->resolutionsPerAuthority[0]->values[0]->value->name;
				}
			}
		}
		return $this->slots;
